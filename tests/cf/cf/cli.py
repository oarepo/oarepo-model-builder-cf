...
import tqdm
import yaml
from cf.proxies import current_service
...
def reindex(recreate, queue, *args, **kwargs):
    ...
    id_generator = (
        x[0]
        for x in db.session.query(CfRecord.model_cls.id).filter(
            CfRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = CfRecord.get_record(rec_id)
